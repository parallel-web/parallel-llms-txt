{
  ...
  "paths": {
      ...
      "post": {
        ...
        "parameters": [
          ...
          },
          {
            ...
            "in": "query",
            "required": false,
            "description": "When set to true (default), the response includes the latest status of the runs. Set to false to skip status refresh and just add runs to the queue, which is useful when adding tasks to a task group at scale.",
            "schema": {
              "type": "boolean",
              "default": true,
    ...
    "/v1alpha/monitors/list": {
      "get": {
        ...
        "operationId": "list_monitors_paginated_v1alpha_monitors_list_get",
        "parameters": [
          {
            "name": "cursor",
            "in": "query",
            ...
            "schema": {
              ...
              "description": "Opaque pagination token returned as `next_cursor` in a previous response. Omit to start from the most recently created monitor.",
              "title": "Cursor"
            },
            "description": "Opaque pagination token returned as `next_cursor` in a previous response. Omit to start from the most recently created monitor."
          },
    ...
    "schemas": {
      ...
      "TaskGroupRunRequest": {
        "properties": {
          ...
          "refresh_status": {
            "type": "boolean",
            "title": "Refresh Status",
            "description": "Whether to refresh and return the latest status of all runs in the response. Defaults to `true`. Set to `false` when adding tasks at scale to get faster responses. The response will include a cached status instead. You can retrieve the latest status separately via the GET task-group endpoint.",
            "default": true
          }
        },
        "type": "object",
      ...
      "TaskGroupRunResponse": {
        "properties": {
          "status": {
            "$ref": "#/components/schemas/TaskGroupStatus",
            "description": "Status of the group. When `refresh_status` is `true` (the default), this returns the latest status. When `refresh_status` is `false`, this returns a cached status."
          },
          "run_ids": {
            "items": {
              "type": "string"
      ...
      "UpdateMonitorRequest": {
        "properties": {
          ...
          "frequency": {
            ...
            "title": "Frequency",
            "description": "Updated frequency of the monitor. Format: '<number><unit>' where unit is 'h' (hours), 'd' (days), or 'w' (weeks). Must be between 1h and 30d (inclusive).",
            "examples": [
              "1d",
              "1w",
              "1h",
  ...
  "tags": [
    ...
    {
      "name": "Monitor",
      "description": "The Monitor API enables continuous web monitoring for material changes. Define monitoring queries with scheduled cadences (hourly, daily, weekly); the service periodically searches the web, detects changes, deduplicates events, and delivers notifications via webhooks or Slack.\n- Create, update, and delete monitors with custom output schemas\n- Retrieve detected events and event groups\n- Simulate events for testing"
    },
    {
      "name": "Chat API (Beta)",
