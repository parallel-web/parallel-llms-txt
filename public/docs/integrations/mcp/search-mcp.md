...
The Parallel Search MCP Server provides drop-in web search capabilities for any MCP-aware model. The tool invokes the [Search API](/search/search-quickstart) endpoint but presents a simpler interface to ensure effective use by agents. The tool uses the Search API in `agentic` mode, which returns more concise results than the default `one-shot` mode.

The Search MCP currently comprises a single tool called **web\_search\_preview**.
