...
<Note>
  These examples set `mode` to `"fast"` for predictable low-latency tool responses. See [Search Modes](/search/modes) for other options. You can also configure `excerpts`, `max_results`, and other parameters server-side — see the [API Reference](/api-reference/search/search) for the full list.
</Note>

## Handling the `web_fetch` tool call

When your LLM invokes `web_fetch`, forward `urls` and map `target_content` to `objective` on the [Extract API](/api-reference/extract/extract). The examples below request excerpts (focused markdown) and omit full page content for more token-efficient tool responses; adjust `excerpts` and `full_content` server-side as needed.
