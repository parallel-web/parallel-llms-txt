...
response = requests.post(
    ...
    json={
        "objective": "Series B SaaS companies that hired a VP of Revenue Operations in Q1 2025 and posted 5+ sales roles in the last 60 days",
        "entity_type": "companies",
        "generator": "core",
        "match_conditions": [
            {"name": "series_b_saas", "description": "SaaS company that has raised a Series B."},
            {"name": "revops_hire", "description": "Hired a VP of Revenue Operations in Q1 2025."},
            {"name": "sales_hiring", "description": "Posted 5 or more sales roles in the last 60 days."}
        ],
        "match_limit": 100
    }
)

# Add enrichment columns (website, recent funding, sales headcount growth)
# via POST /v1beta/findall/runs/{findall_id}/enrich
```
...
response = requests.post(
    ...
    json={
        ...
        "search_queries": ["fleet management AI", "logistics automation ML"],
        "advanced_settings": {"max_results": 50}
    }
)

# Returns: ranked URLs, page titles, publish dates, compressed excerpts
```
...
Parallel's [Task API](https://parallel.ai/products/task) runs this kind of structured research:
        ...
        "task_spec": {
            "output_schema": {
                ...
                "json_schema": {
                    "type": "object",
                    "properties": {
                        "has_dedicated_revops": {"type": "boolean"},
                        "estimated_sales_team_size": {"type": "string"},
...
**Job posting spikes:** A company hiring five or more sales reps in a quarter is scaling GTM and likely buying supporting tools. Hiring patterns visible on [LinkedIn](https://www.linkedin.com) and Indeed predict tool purchases months in advance.
...
Sources for these signals include [Crunchbase](https://www.crunchbase.com) for funding data, LinkedIn for job postings and leadership changes, company newsrooms for press releases, [G2](https://www.g2.com) for review activity, and SEC filings for public companies.
...
response = requests.post(
    ...
    json={
        ...
        "webhook": {"url": "https://your-app.com/webhooks/lead-events"}
    }
)

# Webhook notifications fire as new events are detected, with the
...
CRM + Slack/Email → Sales action
     ↓
Won accounts → Feed back into discovery
```
...
The highest-converting signals are: new funding announcements, leadership hires (especially VP-level GTM roles), job posting spikes in sales or engineering, product launches, and technology stack changes visible in job descriptions. These signals indicate budget availability, strategic change, or active vendor evaluation, all of which increase response rates.
