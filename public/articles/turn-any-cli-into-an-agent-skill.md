...
Most of the [most-installed agent skills](https://parallel.ai/articles/best-clawhub-skills-for-openclaw) follow the same shape: a page of markdown teaching the agent an existing CLI. Gog wraps a Google Workspace CLI, Wacli wraps WhatsApp, the GitHub skill drives `gh`. The CLI carries the capability; the [skill](https://parallel.ai/articles/what-are-agent-skills) tells the agent when and how to use it. This tutorial builds one end to end, using our own Parallel CLI as the example, and the same steps apply to any tool you want your agent to use well.

## Why this pattern wins

A skill costs almost nothing in context until used (only its one-line description is always loaded), runs through the shell so the agent can compose it with pipes and scripts, and, since `SKILL.md` became an open standard, works unchanged across Claude Code, Codex CLI, Cursor, OpenCode, OpenClaw, and Hermes. The tradeoffs against MCP servers are covered in our [decision framework](https://parallel.ai/articles/mcp-vs-skills-vs-clis). In brief, local, composable, CLI-shaped capabilities belong in skills.

## Step 1: make sure the CLI is agent-friendly

Agents can't answer interactive prompts, they parse output programmatically, and they decide what to do next from exit codes. Before writing the skill, check the CLI against four properties, and if you're building the CLI too, design them in:
...
The [Parallel CLI](https://docs.parallel.ai/integrations/cli) was built with agents as the intended driver, which is why it makes a clean example: every command takes `--json`, exit codes distinguish bad input (2), auth errors (3), API errors (4), and timeouts (5), and queries pipe in via stdin.

## Step 2: write the `SKILL.md`

A good CLI skill has four parts: frontmatter that tells the agent when to reach for it, setup instructions it can verify, the commands with real examples, and guidance on interpreting results. A complete, working example:
...
**Should the skill install the CLI itself?** Give the agent verified install instructions and let it run them with the user's approval, as the example does, rather than installing silently.

**One big skill or several small ones?** Split when the triggers differ. Search-and-read belongs together; bulk enrichment triggers on different tasks and deserves its own description line. That's why we ship four ClawHub skills rather than one.
...
Copy the example above, swap in your CLI's commands, and test the three failure modes. If you want to start with ours, the [Parallel CLI installs in one line](https://docs.parallel.ai/integrations/cli) and its free tier includes $5 in free credits every month, so the skill works before you've spent anything.
