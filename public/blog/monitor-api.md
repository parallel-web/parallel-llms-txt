...
This GA release introduces two processor tiers (Lite and Base), two run types (snapshot and event stream), Basis on every event, advanced controls like source policy and geo settings, and seamless chaining with our APIs via [Interactions](https://parallel.ai/blog/task-api-interactions).
...
- `**type=event_stream**` returns a discrete event each time new matching information appears, with structured fields (`event_id`, `event_group_id`, `event_date`, `event_type`, and an `output` object containing `type`, `content`, and `basis`). The event's description is found in `output.content`. 
  - URLs act as supporting evidence for the whole detection and are found within `output.basis[].citations[].url`. 
...
![](https://cdn.sanity.io/images/5hzduz3y/production/c5bb8863f788cf33c8f022214debdda92a66693c-5396x3240.jpg)
...
Together, [Interactions](https://docs.parallel.ai/task-api/guides/interactions) and [Basis](https://docs.parallel.ai/monitor-api/monitor-events#event-basis) make Monitor → Task chaining a first-class production pattern.

## **Advanced settings: source policy and location controls**

Two retrieval controls round out the API. **[Source policy](https://docs.parallel.ai/resources/source-policy)** restricts or prioritizes specific domains, eliminating false positives from low-authority sources where signal validity depends on origin (SEC filings, government procurement portals, specific trade publications). **[Location parameters](https://docs.parallel.ai/api-reference/search/search#body-advanced-settings-one-of-0-location-one-of-0)** scope retrieval to specific regions, surfacing the long tail of regional sources.
...
![](https://cdn.sanity.io/images/5hzduz3y/production/7638d73d85ae8b9e967a33fc4d4222cd89f97242-5396x3240.jpg)
...
Get started in our[ Developer Platform](https://platform.parallel.ai/play/monitor) or dive into the[ documentation](https://docs.parallel.ai/monitor-api/monitor-quickstart).
